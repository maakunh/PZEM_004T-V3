...
class functions:
    def conn(self, port):
        ...
        self.master = modbus_rtu.RtuMaster(self.sensor)
    ...
    def send_receive(self, modbus_pdu, slave_addr):
        ...
        # flush the Rx FIFO
        self.sensor.read()
        self.sensor.write(serial_pdu)
        time.sleep(2)

    def reset_energy(self):
        pdu = struct.pack('B', 0X42)
        self.send_receive(pdu, 1)

    def modify_poweralert(self, threshold):
        # Changing power alarm value to 100 W
        self.master.execute(1, cst.WRITE_SINGLE_REGISTER, 1, output_value=threshold)
